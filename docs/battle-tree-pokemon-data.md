...
For already extracted inputs, supply all three exact GARC files instead of a ROM:

```sh
python scripts/generate-battle-tree-pokemon-csv.py \
  --pokemon-garc /path/to/battle_tree_poke.garc \
  --trainer-garc /path/to/battle_tree_trainer.garc \
  --personal-garc /path/to/personal.garc \
  --pokemon-html /path/to/battle-tree-pokemon.html
```

The generator rejects archives whose SHA-256 does not match the analyzed US
retail build, including the personal-data archive.
