...
The source-level and retail-bytecode-level specification is in
[docs/battle-ai-full-spec.md](docs/battle-ai-full-spec.md), with a machine-readable
companion at [docs/battle-ai-full-spec.json](docs/battle-ai-full-spec.json). It
contains normalized derived listings for every recovered Pawn source function,
the exact retail-only Intrude and Royal AMX listings, and the 124-command native
boundary index. The generator is
[scripts/generate-battle-ai-full-spec.py](scripts/generate-battle-ai-full-spec.py);
it requires an externally recovered copy of the deleted source files and does
not copy those source files into this repository.
The companion
[scripts/recover-battle-ai-source.py](scripts/recover-battle-ai-source.py)
recovers those files to a caller-selected external directory from the source
repository’s Git object store. For the supplied archive, the pre-deletion
commit is `6451021a8219e6726491675aff0189c31a2fcc46`.
...
The condition-to-score view is in
[docs/battle-ai-score-rules.md](docs/battle-ai-score-rules.md), generated by
[scripts/generate-battle-ai-score-rules.py](scripts/generate-battle-ai-score-rules.py).
